...
function Projects() {
  ...
  useEffect(() => {
    const tagsArray = [
      ...new Set(projectsData.map((item) => item.tags).flat()),
    ];
    setTags(tagsArray);
    const typeArray = [
      ...new Set(projectsData.map((item) => item.projectType)),
    ];
    setType(typeArray);
    const experienceArray = [
      ...new Set(projectsData.map((item) => item.experienceLevel)),
    ];
  ...
  useEffect(() => {
    const filterProjects = () => {
      ...
      if (selectedExperienceLevels.length > 0) {
        filtered = filtered.filter((project) =>
          selectedExperienceLevels.includes(project.experienceLevel)
        );
      }
    ...
    <div className="page">
      ...
      <div className="ending">
        <p>
          ...
          <a
            href="https://github.com/ryan-huntington/ryan-huntington.github.io"
            target="_blank"
            rel="noreferrer"
